...
class MedianConv(nn.Module):
    ...
    >>> g = dgl.add_self_loop(g)
    >>> feat = th.ones(6, 10)
    >>> conv = MedianConv(10, 2, norm='both', weight=True, bias=True)
    >>> res = conv(g, feat)
    >>> print(res)
    ...
    >>> g = dgl.graph(([0,1,2,3,2,5], [1,2,3,4,0,3]))
    >>> conv = MedianConv(10, 2, norm='both', weight=True, bias=True, allow_zero_in_degree=True)
    >>> res = conv(g, feat)
    >>> print(res)
    ...
    >>> u_fea = th.rand(2, 5)
    >>> v_fea = th.rand(4, 5)
    >>> conv = MedianConv(5, 2, norm='both', weight=True, bias=True)
    ...
    def forward(self, graph, feat, weight=None, edge_weight=None):
        ...
        * Weight shape: :math:`(\text{in_feats}, \text{out_feats})`.
        ...
        with graph.local_scope():
            ...
            feat_src, feat_dst = expand_as_pair(feat, graph)  # Expand features to src/dst features.
            if self._norm in ['left', 'both']:
                degs = graph.out_degrees().float().clamp(min=1)         # return min(max(x, minVal), maxVal)
                if self._norm == 'both':
                    norm = th.pow(degs, -0.5)   # :math:`d_{out}^{-1/2}`
                else:
                    norm = 1.0 / degs           # random walk
                shp = norm.shape + (1,) * (feat_src.dim() - 1)          # (N) + (1,) -> (N, 1)
                norm = th.reshape(norm, shp)
                feat_src = feat_src * norm                              # (N, x_in) -> (N, x_in)
            ...
            if self._in_feats > self._out_feats:
                ...
                graph.srcdata['h'] = feat_src                   # (N, x_out)
                graph.update_all(aggregate_fn, median_reduce)
                rst = graph.dstdata['h']
            else:
                # aggregate first then mult W
                graph.srcdata['h'] = feat_src                   # (N, x_in)
                graph.update_all(aggregate_fn, median_reduce)
                rst = graph.dstdata['h']
                if weight is not None:
                    rst = th.matmul(rst, weight)                # (N, x_out)

            if self._norm in ['right', 'both']:
                degs = graph.in_degrees().float().clamp(min=1)
                if self._norm == 'both':
                    norm = th.pow(degs, -0.5)   # :math:`d_{in}^{-1/2}`
                else:
                    norm = 1.0 / degs           # mean
...
class MedianGCN(nn.Module):
    def __init__(self, in_feats, n_hidden, n_classes, n_layers, activation, dropout):
        ...
        self.layers.append(MedianConv(n_hidden, n_classes))
